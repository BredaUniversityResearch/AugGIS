...
namespace ColourPalette
{
    ...
    public class SingleSetColourPalette : SerializedScriptableObject
    {
		...
		public void ForceActivateSet()
		{
			...
			foreach (CustomImage colourHolder in GameObject.FindObjectsByType<CustomImage>(FindObjectsSortMode.None))
				colourHolder.ColourPaletteChanged();
			foreach (CustomText colourHolder in GameObject.FindObjectsByType<CustomText>(FindObjectsSortMode.None))
				colourHolder.ColourPaletteChanged();
		}
	}
}
