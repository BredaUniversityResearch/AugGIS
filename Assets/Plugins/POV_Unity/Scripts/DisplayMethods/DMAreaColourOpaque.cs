...
namespace POV_Unity
{
	public class DMAreaColourOpaque : AVectorDisplayMethod
	{
		...
		protected override IDisplayMethodRenderData DisplayVectorLayer(VectorLayer a_layer, GameObject a_displayMethodRoot)
		{
			...
			string polyTexture = GetVariable<string>("texture", a_layer, a_layer.data[0]);

			MeshRenderer renderer = a_displayMethodRoot.AddComponent<MeshRenderer>();
			renderer.shadowCastingMode = UnityEngine.Rendering.ShadowCastingMode.Off;
			renderer.lightProbeUsage = UnityEngine.Rendering.LightProbeUsage.Off;
			Material mat = AssetManager.GetOpaqueMaterialForTextureColour(polyTexture, Color.white);
		...
		public void CreatePolygon(VectorLayer a_layer, VectorObject a_polygon, Transform a_parent)
		{
			...
			string polyTexture = GetVariable<string>("texture", a_layer, a_polygon);
			...
			foreach (VectorShape shape in a_polygon.Shapes)
			{
				...
				try
				{
					if (!Util.PolyMeshTryGenerateMesh(poly, out Mesh mesh))
					{
				...
				renderer.shadowCastingMode = UnityEngine.Rendering.ShadowCastingMode.Off;
				renderer.lightProbeUsage = UnityEngine.Rendering.LightProbeUsage.Off;
				Material mat = AssetManager.GetOpaqueMaterialForTextureColour(polyTexture, polyColour);
				a_layer.RegisterLayerMaterial(mat);
				renderer.sharedMaterial = mat;
			}
		}
