...
namespace PassthroughCameraSamples
{
	public static class PassthroughCameraUtils
	{
		...
		private static readonly Pose?[] s_cachedCameraPosesRelativeToHead = new Pose?[2];
		...
		public static bool IsSupported
		{
			get
			{
				...
				{
					return (!HorizonOSVersion.HasValue || HorizonOSVersion >= MINSUPPORTOSVERSION);
				}
		...
		static UnityEngine.XR.InputDevice m_hmd ;
		public static UnityEngine.XR.InputDevice HMD
		{
			get
			{
				if (!m_hmd.isValid)
				{
					List<UnityEngine.XR.InputDevice> connectedXRDevices = new List<UnityEngine.XR.InputDevice>();
					UnityEngine.XR.InputDevices.GetDevices(connectedXRDevices);

					if (connectedXRDevices.Count > 0)
					{
						m_hmd = connectedXRDevices[0];
					}
				}
		...
		public static Pose GetCameraPoseInWorld(PassthroughCameraEye cameraEye)
		{
			...
			if (s_cachedCameraPosesRelativeToHead[index] == null)
			{
				...
				s_cachedCameraPosesRelativeToHead[index] = new Pose
				{
					position = p_headFromCamera,
					rotation = q_headFromCamera
				};
			}
			...
			var worldFromHead = new Pose(pos, rot);
			var worldFromCamera = headFromCamera.GetTransformedBy(worldFromHead); //worldFromHead * headFromCamera;
			worldFromCamera.rotation *= Quaternion.Euler(180, 0, 0);

			return new Pose(worldFromCamera.position, worldFromCamera.rotation);
		}

		//HMD.TryGetFeatureValue(UnityEngine.XR.CommonUsages.centerEyePosition, out Vector3 pos);
		//HMD.TryGetFeatureValue(UnityEngine.XR.CommonUsages.centerEyeRotation, out Quaternion rot);
